...
static int exec_setup(db_t *db, int *out_ctx, int *out_sr, int *out_mr) {
    ...
    int ctx_id = 0;
    if (acta_db_context_create(db, &ctx, &ctx_id) != 0) return -1;
    ...
    int skill_id = 0;
    if (acta_db_skill_create(db, &sk, &skill_id) != 0) return -1;
    ...
    int model_id = 0;
    if (acta_db_model_create(db, &m, &model_id) != 0) return -1;
...
static int exec_create(db_t *db, int ctx_id, int sr_id, int mr_id,
    ...
    int out_id = 0;
    if (acta_db_execution_create(db, &e, &out_id) != 0) return -1;
    return out_id;
}
...
static void test_exec_create_with_parent(void) {
    ...
    execution_t *child = acta_db_execution_get(db, child_id);
...
static void test_exec_get_nonexistent(void) {
    ...
    execution_t *got = acta_db_execution_get(db, 999999);
...
static void test_exec_start_completed(void) {
    ...
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, eid), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_complete(db, eid, "done"), 0);
...
static void test_exec_start_failed(void) {
    ...
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, eid), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_fail(db, eid, "oops"), 0);
...
static void test_exec_complete_twice(void) {
    ...
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, eid), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_complete(db, eid, "first"), 0);
...
static void test_exec_list_by_status_pending(void) {
    ...
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, r1), 0);
...
static void test_exec_list_by_status_completed(void) {
    ...
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, c1), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_complete(db, c1, "ok"), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_start(db, c2), 0);
    TEST_ASSERT_EQ_INT(acta_db_execution_complete(db, c2, "ok"), 0);
...
static void test_exec_list_by_status_invalid(void) {
    ...
    int count = 0;
    execution_t *items = acta_db_execution_list_by_status(db, "bogus", &count);
    (void)items;
    /* Either returns 0 results or an error is signaled via count == 0 */
    TEST_ASSERT_EQ_INT(count, 0);
    if (items) acta_db_execution_list_free(items, count);
...
static void test_exec_list_children_with(void) {
    ...
    int count = 0;
    execution_t *children = acta_db_execution_list_children(db, parent_id, &count);
    TEST_ASSERT_NOT_NULL(children);
    TEST_ASSERT_EQ_INT(count, 3);
    for (int i = 0; i < count; i++) {
        TEST_ASSERT_EQ_INT(children[i].parent_execution_id, parent_id);
    }
    acta_db_execution_list_free(children, count);
...
static void test_exec_list_children_none(void) {
    ...
    int count = 0;
    execution_t *children = acta_db_execution_list_children(db, leaf_id, &count);
    (void)children;
    TEST_ASSERT_EQ_INT(count, 0);
    if (children) acta_db_execution_list_free(children, count);
...
static void test_exec_list_children_nonexistent(void) {
    ...
    int count = 0;
    execution_t *children = acta_db_execution_list_children(db, 999999, &count);
    (void)children;
    /* Either 0 results or error */
    TEST_ASSERT_EQ_INT(count, 0);
    if (children) acta_db_execution_list_free(children, count);
...
static void test_exec_raw_insert(void) {
    ...
    execution_t *got = acta_db_execution_get(db, 42);
    TEST_ASSERT_NOT_NULL(got);
