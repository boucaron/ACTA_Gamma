...
skill_folder_t *acta_db_skill_folder_get(db_t *db, int id) {
    if (!db) return NULL;
    const char *sql = "SELECT id, name, parent_id, created_at, updated_at, deleted_at FROM skill_folders WHERE id = ?;";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db->handle, sql, -1, &stmt, NULL) != SQLITE_OK) return NULL;
    sqlite3_bind_int(stmt, 1, id);
    skill_folder_t *result = NULL;
    if (sqlite3_step(stmt) == SQLITE_ROW) result = row_to_skill_folder(stmt);
    sqlite3_finalize(stmt);
    return result;
}
...
skill_folder_t *acta_db_skill_folder_list_children(db_t *db, int parent_id, int *out_count) {
    ...
    const char *sql = parent_id == 0
        ? "SELECT id, name, parent_id, created_at, updated_at, deleted_at FROM skill_folders WHERE parent_id IS NULL AND deleted_at IS NULL ORDER BY name;"
        : "SELECT id, name, parent_id, created_at, updated_at, deleted_at FROM skill_folders WHERE parent_id = ? AND deleted_at IS NULL ORDER BY name;";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db->handle, sql, -1, &stmt, NULL) != SQLITE_OK) return NULL;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (!tmp) { acta_db_skill_folder_free(item); sqlite3_finalize(stmt); return NULL; }
        items = tmp;
        items[count++] = *item;
        free(item);
    }
...
skill_folder_t *acta_db_skill_folder_list_all(db_t *db, int *out_count) {
    if (!db || !out_count) return NULL;
    const char *sql = "SELECT id, name, parent_id, created_at, updated_at, deleted_at FROM skill_folders WHERE deleted_at IS NULL ORDER BY name;";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db->handle, sql, -1, &stmt, NULL) != SQLITE_OK) return NULL;
...
void acta_db_skill_folder_free(skill_folder_t *f) {
    if (!f) return;
    free(f->name); free(f->created_at); free(f->updated_at); free(f->deleted_at); free(f);
}
...
skill_t *acta_db_skill_get(db_t *db, int id) {
    ...
    sqlite3_bind_int(stmt, 1, id);
    skill_t *result = NULL;
    if (sqlite3_step(stmt) == SQLITE_ROW) result = row_to_skill(stmt);
    sqlite3_finalize(stmt);
    return result;
}
...
skill_t *acta_db_skill_list_in_folder(db_t *db, int folder_id, int *out_count) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (!tmp) { acta_db_skill_free(item); sqlite3_finalize(stmt); return NULL; }
        items = tmp;
        items[count++] = *item;
        free(item);
    }
...
void acta_db_skill_free(skill_t *s) {
    ...
    free(s->output_schema); free(s->created_at); free(s->updated_at); free(s->deleted_at); free(s);
}
