...
- **Not thread-safe by design.** One `db_t` is one SQLite connection, owned by a single thread. Concurrency is not shared through the library: the runner uses its own connection, the GUI uses its own connection (the in-app runner thread opens its own handle), and the CLI opens its own. WAL makes cross-process read/write work, but simultaneous writers on the same file are not supported in the current implementation — single-writer is still the expected pattern. There is a 5 s `busy_timeout` (the `ACTA_DB_BUSY_TIMEOUT_MS` constant in `acta_db/src/db.c`, set on every open): a transient contention (a WAL checkpoint, a concurrent `db backup`) is absorbed by waiting up to 5 s instead of failing mid-pipeline. If a write still fails with `SQLITE_BUSY` after the timeout, `acta_db` surfaces a wrapped diagnostic in `acta_db_last_error()`: `SQLITE_BUSY: concurrent write detected; this database is single-writer by design — wait for the other process to finish and rerun` (the losing operation is aborted, nothing is partially written, and the other writer's committed data is intact) — there is no automatic retry, so the operator's action is to rerun the command.
