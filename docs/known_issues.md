...
| KI-2 | **Unknown JSON keys silently accepted** on `create` — `{"type","content","bogus_key"}` creates the row. No key allow-list anywhere. | open | **fail** | `echo '{"type":"x","content":"c","bogus_key":1}' \| acta_cli context create --stdin` → rc 4 "unknown key" (actual: rc 0, row created). |
| KI-3 | `--raw_out` on a **NULL** field → rc 10 `unknown --raw_out field: 'metadata'` while listing `metadata` as supported; help says "null values produce no output". NULL value conflated with unknown field in `context.c`/`execution.c` (`else if(v)`). | open | **fail** | `acta_cli context get <id-without-metadata> --raw_out metadata` → rc 0, empty output (actual rc 10). |
| KI-4 | Trailing `help` form **silently executes** the action: `context list help` runs the list ("help" swallowed as an unconsumed positional). | open | **fail** | `acta_cli context list help` → help text (actual: full JSON list, rc 0). |
...
- `tests/context/context_test_misc.c`: `test_get_raw_out_null_field` (KI-3,
  red), `test_trailing_help_runs_action` (KI-4, red),
  `test_create_unknown_json_key` (KI-2, red), `test_list_id_only_pinned`
  (KI-6, pin).
...
Run: `make -C acta_cli test`. Currently red: KI-2, KI-3, KI-4 (context suite)
— one per open bug; when fixing one, update its test to assert the new
behavior and close the row here.
