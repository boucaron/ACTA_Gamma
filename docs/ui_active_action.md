...
| # | Action | Source | Notes / dependencies |
|---|--------|--------|----------------------|
| P4 | `editExecution` err handling — `qWarning` on failed `context_get` / `skill_revision_get` / `model_revision_get` lookups in `ExecutionDialog::editExecution` (failures previously left fields silently blank) | UR #14 | queued; ~9-line mechanical fix following the codebase pattern already used in the log list and panels |
