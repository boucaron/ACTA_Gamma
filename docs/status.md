...
- Soft-delete lifecycle for contexts and executions: complete at the DB layer (`acta_db`) — `delete` / `restore` flag flips, live-only lister defaults, `_with_deleted` / `include_deleted` variants, `create` refuses deleted contexts, `reset` refuses deleted executions; now exposed in the CLI (`acta_cli`: `context delete/restore`, `exec delete/restore`, `--include_deleted` / `--deleted` on `get` / `list` / `count`, `deleted_at` in JSON/table output) and in `docs/cli_spec.md` — see `docs/soft_delete_context_execution.md` and `docs/soft_delete_context_execution_acta_cli_runner.md`; remaining: `tools.c` (T3) rows, the `acta_runner` single-claim guard, CLI/runner tests, and GUI exposure.
