...
**Not implemented by decision:** automatic retries are out of scope (owner decision, 2026-09-12 — retrying without knowing the cause is pointless); a failed execution is retried manually via the `failed → pending` reset (GUI Retry button / `acta_cli exec reset <id>`). Streaming responses are not needed (owner decision, 2026-07-10).
