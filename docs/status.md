...
- Soft-delete lifecycle for contexts and executions at the DB layer (`acta_db`): `delete` / `restore` flag flips, live-only lister defaults, `_with_deleted` / `include_deleted` variants, `create` refuses deleted contexts, `reset` refuses deleted executions — see `docs/soft_delete_context_execution.md`; CLI/GUI/runner exposure is the follow-up phase.
