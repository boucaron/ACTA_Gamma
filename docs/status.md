...
- Soft-delete lifecycle for contexts and executions: complete at the DB layer (`acta_db`) — `delete` / `restore` flag flips, live-only lister defaults, `_with_deleted` / `include_deleted` variants, `create` refuses deleted contexts, `reset` refuses deleted executions; exposed in the CLI (`acta_cli`: `context delete/restore`, `exec delete/restore`, `--include_deleted` / `--deleted` on `get` / `list` / `count`, `deleted_at` in JSON/table output, `tools.c` (T3) rows, deleted test suites) and in `docs/cli_spec.md`; the `acta_runner` single-claim `deleted_at` guard and its claim test suite are in — see `docs/soft_delete_context_execution.md` and `docs/soft_delete_context_execution_acta_cli_runner.md` (all sections done); remaining: GUI exposure.
