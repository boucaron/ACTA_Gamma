...
**You need:** a C compiler (gcc/clang), SQLite, curl, cJSON (plus Qt 6 only for the optional GUI), and a running llama.cpp `llama-server` in router mode (launched without `-m`, with `--models-dir` pointing at a folder of GGUF files — defined in [Quick start](#quick-start)) serving at least one GGUF model. Everything else is in this repo — if you just want a run on screen, skip to [Quick start](#quick-start) (step 2 there covers the optional `make gui` build).
...
Terms used throughout this README: a **skill** is a versioned prompt template with an optional output schema — it is not a tool, function, or agent capability; a **context** is a named, immutable snapshot of input data (a document, a code file, a log excerpt) — it is not the model's prompt window; a **revision** is an immutable snapshot of a skill's or model's fields, created automatically whenever the parent is created or edited — an execution is always bound to a specific revision, not to the mutable parent.
...
- **Revisions.** Skill folders and model folders are optional organizational groupings for list display; they do not affect execution. Creating, updating, or soft-deleting a skill or model parent auto-snapshots a new immutable revision row via a DB trigger — a skill revision captures its `name`, `description`, `prompt_template`, and `output_schema`; a model revision captures its `name`, `description`, `backend`, `base_url`, `model_identifier`, and `configuration`. There is no separate "snapshot" command, and no `active`/`current` flag: the "current" revision is simply the latest one. Editing *creates, not modifies*: existing executions keep pointing at the revision they were bound to.
...
- **Concurrency.** The claim is atomic: `start()` is a single conditional `UPDATE … WHERE id=? AND status='pending'`, so two processes can never both own the same execution. Single-writer by design: the database runs in WAL mode, concurrent readers are fine, but simultaneous *writers* on the same file are **not supported** — a 5 s `busy_timeout` absorbs transient contention, and if a write still fails with `SQLITE_BUSY`, the operator's action is simply to rerun the command. The exact diagnostic, the constant, and the WAL details are in [`docs/DBDesign.md`](docs/DBDesign.md).
...
The runner builds the chat call from the bound revisions: `system` = the skill's `prompt_template`, `user` = `context.content` — there is no per-execution prompt field, and an empty context content fails the execution at preflight, before any backend call. Preflight enforces a deterministic `max_chars` guard on the two strings before any backend call (default 100,000, configurable): the count is the **UTF-8 byte length** of `prompt_template` + `context.content` (equivalently `strlen` in C, which counts bytes), not Unicode codepoints and not tokens — multibyte text (CJK, emoji) consumes more bytes per character, and the backend's own context window (the router's `-c`, in tokens) is a separate, final constraint. Treat the guard as a coarse pre-check, not a guarantee: a prompt can pass it and still be rejected by the backend for exceeding `-c` — size `-c` to your model and expect that overflow to surface as a backend error, not a preflight failure. Full pipeline — claim, resolve, preflight, call, validate, record — and the backend contract are in [`docs/runner_contract.md`](docs/runner_contract.md); the llama.cpp router surface is in [`docs/llamacpp_server_contract.md`](docs/llamacpp_server_contract.md). There is no prompt-injection defense: the context reaches the model verbatim, and the model has no boundary between data and instruction inside it. The tool assumes the operator vetted the input — if you feed untrusted content (logs, user-submitted text, crawled documents), you are responsible for the model's behavior on it; and note that `output_schema` validation checks the response's *structure*, not its intent — an injected instruction that makes the model answer with schema-valid but wrong content still passes.
...
| `acta_gui/` | C++ / Qt 6 (Core, Widgets) | Desktop GUI: manage skills, models, contexts, review executions, and run them. The in-app **Run** button runs the runner's pipeline in-process (single pipeline codebase — the source coupling is a hard build constraint, see [`docs/building.md`](docs/building.md), "GUI–runner source coupling"). While a run is in flight **Run** becomes **Cancel**: cancel cooperatively stops the pipeline and transitions the row to `cancelled`, but an HTTP call already in flight runs to completion — the tokens are consumed regardless; cancel stops the **recording**, not the compute (full mechanics in [`docs/runner_contract.md`](docs/runner_contract.md), decision 1) |

The backend is a llama.cpp `llama-server` running in **router mode** (launched without a model, e.g. with `--models-dir` pointing at local GGUF files) — it is the **only** supported backend. llama.cpp **0.5.0** (router mode) is the tested baseline and a hard requirement: the OpenAI-compatible surface is pinned to that exact version's behavior, the runner does no version negotiation, and any other version (0.4.x, 0.5.1+, 1.0) is untested and unsupported. Models are loaded when the server starts — **restart the server after adding or removing model files**; dropping a new GGUF into `--models-dir` while the server runs is not covered by the contract. The pipeline talks to it through the OpenAI-compatible HTTP surface (`GET /health`, `GET /v1/models`, `POST /v1/chat/completions`), but that surface is the interface, not a portability promise; ACTA Gamma is not a generic OpenAI client. The backend is assumed to be a **local, unauthenticated, plain-HTTP** endpoint — do not point `base_url` at a remote host; the API key (if any) travels in cleartext.
...
3. **Start the backend** — a llama.cpp `llama-server` in **router mode** (launched **without** `-m`: every GGUF in `--models-dir` becomes a served model and each request is routed to the matching one):
   ...
   This serves every GGUF in `models/` at `http://127.0.0.1:8080` — it is the llama.cpp router, not a generic OpenAI endpoint (full contract: [`docs/llamacpp_server_contract.md`](docs/llamacpp_server_contract.md) §1). One model is fine: point `--models-dir` at a folder containing that one GGUF — GGUF model files are downloadable, e.g., from Hugging Face (a small one to start with: `huggingface-cli download Qwen/Qwen2.5-0.5B-Instruct-GGUF --include "*q4_k_m.gguf" --local-dir models/`); if you don't have llama.cpp yet, [`docs/building.md`](docs/building.md) covers install/build. The `-c 2048` above is a **demo value** — it is the backend's *token* context window. Set `-c` to match your model's supported context length.

> **Note:** `max_chars` (default 100,000) counts UTF-8 *bytes* of `prompt_template` + `context.content` — it is a preflight guard, not a token check. The backend's `-c` (in tokens) is a separate, independent constraint: a request can pass preflight and still be rejected by the backend.

   Before any run you can verify the backend and a model **without consuming tokens** — `acta_runner check <model-record-id>` does the two token-free preflight calls (`GET /health`, `GET /v1/models`) and reports `ok` / the served `max_context`, or a distinct verdict (`model still loading`, `server unreachable`, `model not served`, `catalog unreachable`), with no execution row and no DB write ([`docs/runner_contract.md`](docs/runner_contract.md), "check action"). `run` does this automatically before the claim: `acta_runner run <id>` and `run --pending` call the same two token-free GETs before anything is claimed, and a dead backend or an unserved model exits with the `check` verdict code (12/13) and the same JSON verdict line — nothing is claimed, no DB write. `check` remains the manual triage tool: probing a server before any model is registered (standalone mode), or after a `failed` backend call.
...
* **`ACTA_Gamma.conf`** — the per-machine config file: a flat JSON object with at most the four keys below, in the same app-data directory as the default DB file. All three binaries read it through the same helper; a malformed file or an unknown key is a fail-closed hard error. It may hold an `api_key` at rest, so treat it as sensitive — keeping it private is the owner's job, not the app's: on POSIX the app refuses a file that is not user-only readable (mode `0600`, hard error), but on Windows the mode bits are meaningless and a warning is printed and the file is read anyway (so on a shared Windows machine, an `api_key` in the file can be read by other users) — **on a shared Windows machine, do not store an `api_key` in the config file; set `$OPENAI_API_KEY` instead**.
...
#    see the note below: only a subset of JSON Schema keywords is enforced)
...
#    (`run` does this preflight automatically before the claim — see the
#    paragraph above; standalone `check` is also the way to probe a server
#    before any model is registered)
acta_runner check 1
...
A failed backend call (server down, connection error, or the `--timeout` exceeded) leaves the execution in `failed` with the error recorded in `error`.

| Scenario | What happens | Recovery |
|---|---|---|
| Backend down / timeout | execution `failed`, error recorded | `acta_cli exec reset 1` (`failed → pending`) → `acta_runner run 1` again (or the GUI Retry button, which does both) |
| Clean exit (Ctrl+C / SIGTERM) while `running` | `failed` + the diagnostic `runner process exited during execution` and a matching `execution_failed` log row (death marker) | `exec reset` → `run` |
| Hard kill (SIGKILL, power loss) | row left orphaned in `running` | `acta_runner sweep --stale-seconds N`, then `exec reset` → `run` |
| Hung run (process alive, HTTP call stuck) | looks stale once its last activity is older than N | `sweep --stale-seconds N` when you suspect a crash or a hung run — it is not a daemon and nothing runs it for you |

`sweep` fails executions left in `running` whose last activity — the newest of its `execution_log` rows and `started_at` — is older than N. Pick N larger than the longest legitimate run you may have in flight (e.g. `--timeout 600` → `--stale-seconds 650` or more) so a live run is never swept. Note the limit: sweep judges staleness by last activity, not by process liveness — a runner that is alive but stuck in a hanging HTTP call can look stale; if you run very long calls, use a larger N or check the runner process before sweeping (details in [`docs/runner_contract.md`](docs/runner_contract.md), decision 6). The automatic pre-claim preflight and the manual `check` triage are described in the [Quick start](#quick-start).
...
(`sha256.c`). It is used for the `context create` content hash (the
context's `hash` field, computed as SHA-256 of the content, lowercase
hex, when the hash is omitted). This code is released into the public
domain free of any restrictions. The author requests acknowledgement if the code is used,
