...
| Multi-model via a llama.cpp `llama-server` router (the documented, tested backend); other OpenAI-compatible backends work through the standard surface — see [Implementation](#implementation) | No server manager mode — the backend is user-launched and user-managed ([`runner_contract.md`](docs/runner_contract.md)) |
...
**Prompt size limit.** Since the assembled prompt is exactly these two strings, a plain char count is a deterministic, model-agnostic guard: the execution's preflight checks `strlen(prompt_template) + strlen(context.content)` against the `max_chars` limit — the per-machine config file's `"max_chars"` key, falling back to the built-in default of 100,000 chars — and if the total exceeds it, the execution fails preflight with `EXIT_INVALID` ("prompt too large: N chars total (context X + skill prompt Y) exceeds max_chars Z") **before any backend call**. It is a guard, not a window-fit guarantee: for prompts under the limit the backend's served `max_context` remains the final arbiter (still recorded in the `preflight_passed` log row; it is not used by the check itself).
...
| `acta_gui/` | C++ / Qt 6 (Core, Widgets) | Desktop GUI: manage skills, models, contexts, review executions, and run them (the in-app "Run" button runs the runner's pipeline in-process — it directly compiles and reuses the runner's own source files `acta_runner/src/run.c` and `acta_runner/src/backend.c` via its qmake project, no `acta_runner` binary needed; there is a single pipeline codebase, not a second copy of the pipeline logic — **and the claim step is the single `acta_db_execution_start` in `acta_db` used by both paths, so the atomic `pending → running` claim and the state-transition logic are shared, not duplicated**). While a run is in flight the button toggles into **Cancel**, which cooperatively cancels the in-flight pipeline and transitions the row to `cancelled` |
...
**Which backends are supported.** The backend ACTA Gamma is built, tested, and documented against is the llama.cpp `llama-server` in router mode. The pipeline touches the backend only through the standard OpenAI-compatible surface — `GET /health`, `GET /v1/models`, `POST /v1/chat/completions` — so any other OpenAI-compatible backend works for those calls, provided it serves the model's `model_identifier`. Two steps are llama.cpp-specific, and both degrade gracefully instead of failing:

* the **catalog fetch** (`GET /`, models.json) is best-effort audit data only — a non-llama backend has no catalog, `"catalog":null` is recorded, and the execution is unaffected;
* **server-side schema application** (`response_format: json_schema`) — when the backend does not apply the skill's `output_schema` server-side, the model record sets `supports_response_format: false` and the runner validates the raw response itself after the call.
