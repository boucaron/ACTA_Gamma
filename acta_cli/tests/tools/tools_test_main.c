 ...
 *      14 global_flags, entity_aliases, 3 input_sources, 7 exit codes,
 *      error contract with the `code == -exit` invariant;
 *   3. the tools array has exactly 69 entries covering all 10 entities
 ...
 *      the T3 "flags ⊆ entity_flag_specs" invariant for all 69 entries
...
static void check_structure(stest_ctx_t *ctx, cJSON *root)
{
    ...
    TEST_EQ(ctx, cJSON_GetArraySize(gf), 14);
    ...
    TEST_EQ(ctx, cJSON_GetArraySize(tools), 69);
